...
# Env
from factored_rl.experiments.common import (
    initialize_env,
    initialize_model,
    get_checkpoint_path,
)
...
def main(cfg: configs.Config):
    cfg = configs.initialize_experiment(cfg, "acf")
    ...
    with open(filename, "w") as results_file:
        results = train_agent_on_env(
            agent, env, cfg.env.n_training_episodes, results_file
        )

    if not cfg.loader.load_model:
        ckpt_path = get_checkpoint_path(
            cfg, logs_dirname="pytorch_logs", create_new_version=True
        )
