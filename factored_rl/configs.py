...
class ResidualCNNConfig:
    cnn_blocks: int = MISSING
    depth: int = MISSING
    min_resolution: int = MISSING
    cnn_activation: str = "silu"
    transposed: bool = False
    kernel_size: int = MISSING
    stride: int = MISSING
    mlp_layers: list[int] = MISSING
...
@dataclass
class WMConfig:
    mlp: MLPConfig = immutable(MLPConfig)
    attn: AttnConfig = immutable(AttnConfig)
...
class QNetConfig(MLPConfig):
    basis: BasisConfig = immutable(BasisConfig)


@dataclass
class ModelConfig:
    name: Optional[str] = None
    arch: ArchitectureConfig = immutable(ArchitectureConfig)
    ...
    mlp: MLPConfig = immutable(MLPConfig)
    cnn: CNNConfig = immutable(CNNConfig)
    residualcnn: ResidualCNNConfig = immutable(ResidualCNNConfig)
    wm: WMConfig = immutable(WMConfig)
...
class DQNAgentConfig(AgentConfig):
    ...
    epsilon_half_life_steps: int = (
        MISSING  # number of steps for epsilon to decrease by half
    )
...
class Config:
    ...
    timestamp: bool = (
        True  # Whether to add a timestamp to the experiment directory path
    )
    dir: str = MISSING
    agent: AgentConfig = MISSING
    env: EnvConfig = MISSING
    model: ModelConfig = MISSING
    loss: LossConfig = immutable(LossConfig)
    trainer: TrainerConfig = MISSING
    transform: TransformConfig = MISSING
    tuner: TunerConfig = immutable(TunerConfig)
    loader: LoaderConfig = immutable(LoaderConfig)
    test: bool = False
    verbose: bool = False
...
def _initialize_experiment_dir(cfg: Config) -> str:
    ...
    prefix = (
        "~/data-gdk/csal/factored"
        if platform.system() == "Linux"
        else "~/dev/factored-reps"
    )
    prefix = os.path.expanduser(prefix)
    cfg.dir = (
        f"{prefix}/results/factored_rl/{cfg.experiment}/{cfg.trial}/{cfg.seed:04d}/"
    )
...
def _initialize_device(cfg) -> torch.device:
    device = torch.device(
        "cuda" if (torch.cuda.is_available() and not cfg.disable_gpu) else "cpu"
    )
